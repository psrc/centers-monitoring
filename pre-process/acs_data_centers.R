...
centers_estimate_from_bg <- function(split_df=blockgroup_splits, estimate_df=blockgroups, center_type, split_type, center_name) {
  ...
  if (center_name == "All Centers" & center_type == rgc_title) {cn <- "not in regional growth center"}
  if (center_name == "All Centers" & center_type == mic_title) {cn <- "not in mic"}
...
rgc_modes <- read_csv(rgc_dest_mod_file) |>
  ...
  mutate(grouping = case_when(
    mode == "SOV" ~ "SOV",
    mode %in% c("HOV2", "HOV3+") ~ "non-SOV",
    mode == "Bike" ~ "non-SOV",
    mode %in% c("School Bus", "Transit") ~ "non-SOV",
    mode == "TNC" ~ "non-SOV",
    mode == "Walk" ~ "non-SOV")) |>
...
# All RGCs
all_rgcs <- rgc_modes |> filter(geography_type == rgc_title & geography != "Not in RGC") |> group_by(year, grouping, concept) |> summarise(estimate = sum(estimate)) |> as_tibble() |> mutate(geography = "All Centers", geography_type = rgc_title)
...
# All MICs
all_mics <- rgc_modes |> filter(geography_type == mic_title & geography != "Not in RGC") |> group_by(year, grouping, concept) |> summarise(estimate = sum(estimate)) |> as_tibble() |> mutate(geography = "All Centers", geography_type = mic_title)
...
for(center in c(rgc_names, "All Centers")) {
  ...
  df <- centers_estimate_from_bg(center_type = rgc_title, split_type = "percent_of_total_pop", center_name=center)
...
for(center in c(mic_names, "All Centers")) {
  ...
  df <- centers_estimate_from_bg(center_type = mic_title, split_type = "percent_of_total_pop", center_name=center)
  ifelse(is.null(centers), centers <- df, centers <- bind_rows(centers, df))
  rm(df)
...
for(center in c(rgc_names, "All Centers")) {
  ...
  df <- centers_estimate_from_bg(center_type = rgc_title, split_type = "percent_of_occupied_housing_units", center_name=center)
  ifelse(is.null(centers), centers <- df, centers <- bind_rows(centers, df))
  rm(df)
...
for(center in c(mic_names, "All Centers")) {
  ...
  df <- centers_estimate_from_bg(center_type = mic_title, split_type = "percent_of_occupied_housing_units", center_name=center)
  ifelse(is.null(centers), centers <- df, centers <- bind_rows(centers, df))
  rm(df)
...
for(center in c(rgc_names, "All Centers")) {
  ...
  df <- centers_estimate_from_bg(center_type = rgc_title, split_type = "percent_of_housing_units", center_name=center)
  ifelse(is.null(centers), centers <- df, centers <- bind_rows(centers, df))
  rm(df)
...
for(center in c(mic_names, "All Centers")) {
  ...
  df <- centers_estimate_from_bg(center_type = mic_title, split_type = "percent_of_housing_units", center_name=center)
  ifelse(is.null(centers), centers <- df, centers <- bind_rows(centers, df))
  rm(df)
...
parcel_geo <- parcel_dims |>
  select("parcel_dim_id", "parcel_id", parcel_year="base_year", County="county_name", rgc="regional_growth_center_2023_12_12", mic="manufacturing_industrial_center_2024_01_05") |>
...
parcels <- left_join(parcel_facts, parcel_geo, by=c("parcel_dim_id")) |> filter(estimate_year %in% c(pre_api_year, api_years)) 
